...
fn run_integration_test(path: &std::path::Path) {
    ...
    let mut project = match atlas::resolver::Project::load_and_resolve(path, &stdlib_path) {
        Ok(p) => p,
        Err(errors) => {
            panic!("Project load/resolve errors in {}: {:?}", path.display(), errors);
        }
    };
    ...
    let typed_ast = match typechecker.check_project(&mut project) {
        Ok(t) => t,
        Err(e) => panic!("Typecheck errors in {}: {:?}", path.display(), e),
    };
    ...
    let ir = codegen.generate(&merged).expect("Codegen failed");
    ...
    if !clang_status.status.success() {
        ...
        panic!("Clang compilation failed for {}:\n{}\n{}", path.display(), stdout, stderr);
    }
    ...
    assert_eq!(
        ...
        path.display(), expected_exit_code, actual_exit_code
    );
    ...
    if !expected_output.is_empty() {
        assert_eq!(
            ...
            path.display(), expected_output, actual_output
        );
    }
}

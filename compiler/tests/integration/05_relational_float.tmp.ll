...
define i64 @main() {
entry:
    %x.addr.0 = alloca double
    %y.addr.1 = alloca double
    store double 3.14e0, double* %x.addr.0
    store double 2.71e0, double* %y.addr.1
    %tmp0 = load double, double* %x.addr.0
    %tmp1 = load double, double* %y.addr.1
    %tmp2 = fcmp ogt double %tmp0, %tmp1
    br i1 %tmp2, label %if.then.0, label %if.end.0
if.then.0:
    ret i64 1
if.end.0:
    ret i64 0

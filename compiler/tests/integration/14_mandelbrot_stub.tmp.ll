...
entry:
    ...
    store i64 0, i64* %x.addr.1
    br label %while.cond.0
while.cond.0:
    %tmp0 = load i64, i64* %x.addr.1
    %tmp1 = icmp slt i64 %tmp0, 5
    br i1 %tmp1, label %while.body.0, label %while.end.0
while.body.0:
    %tmp2 = load i64, i64* %count.addr.0
    %tmp3 = load i64, i64* %x.addr.1
    %tmp4 = add i64 %tmp2, %tmp3
    store i64 %tmp4, i64* %count.addr.0
    %tmp5 = load i64, i64* %x.addr.1
    %tmp6 = add i64 %tmp5, 1
    store i64 %tmp6, i64* %x.addr.1
    br label %while.cond.0
while.end.0:
    %tmp7 = load i64, i64* %count.addr.0
    %tmp8 = icmp eq i64 %tmp7, 10
    br i1 %tmp8, label %if.then.1, label %if.end.1
if.then.1:
    %tmp9 = call i32 @"putchar"(i32 42)

...
define i32 @"main"() {
entry:
    %tmp355 = call i8* @"malloc"(i64 8)
    %tmp356 = bitcast i8* %tmp355 to i64*
    %tmp357 = alloca i64*
    store i64* %tmp356, i64** %tmp357
    %tmp358 = load i64*, i64** %tmp357
    %tmp359 = load i64*, i64** %tmp357
    %tmp360 = getelementptr inbounds i64, i64* %tmp359, i64 0
    store i64 42, i64* %tmp360
    %tmp361 = load i64*, i64** %tmp357
    %tmp362 = getelementptr inbounds i64, i64* %tmp361, i64 0
    %tmp363 = alloca i64*
    store i64* %tmp362, i64** %tmp363
    %tmp364 = load i64*, i64** %tmp363
    %tmp365 = load i64*, i64** %tmp363
    %tmp366 = load i64, i64* %tmp365
    %tmp367 = load i64*, i64** %tmp363
    %tmp368 = load i64, i64* %tmp367
    %tmp369 = sub i64 %tmp368, 42
    %tmp370 = trunc i64 %tmp369 to i32
    ret i32 %tmp370
}

...
entry:
    ...
    %tmp0 = load i64, i64* %n.addr
    %tmp1 = icmp sle i64 %tmp0, 1
    br i1 %tmp1, label %if.then.0, label %if.end.0
if.then.0:
    %tmp2 = load i64, i64* %n.addr
    ret i64 %tmp2
if.end.0:
    %tmp3 = load i64, i64* %n.addr
    %tmp4 = sub i64 %tmp3, 1
    %tmp5 = call i64 @"main.fib"(i64 %tmp4)
    %tmp6 = load i64, i64* %n.addr
    %tmp7 = sub i64 %tmp6, 2

...
entry:
    %a.addr.0 = alloca i64*
    %b.addr.1 = alloca i64*
    %res.addr.2 = alloca i64
    %tmp0 = call i8* @"malloc"(i64 8)
    %tmp1 = bitcast i8* %tmp0 to i64*
    store i64* %tmp1, i64** %a.addr.0
    %tmp2 = call i8* @"malloc"(i64 8)
    %tmp3 = bitcast i8* %tmp2 to i64*
    store i64* %tmp3, i64** %b.addr.1
    %tmp4 = load i64*, i64** %a.addr.0
    %tmp5 = getelementptr i64, i64* %tmp4, i64 0
    store i64 42, i64* %tmp5
    %tmp6 = load i64*, i64** %b.addr.1
    %tmp7 = bitcast i64* %tmp6 to i8*
    %tmp8 = load i64*, i64** %a.addr.0
    %tmp9 = bitcast i64* %tmp8 to i8*
    %tmp10 = call i8* @"memcpy"(i8* %tmp7, i8* %tmp9, i64 4)
    %tmp11 = load i64*, i64** %b.addr.1
    %tmp12 = getelementptr i64, i64* %tmp11, i64 0
    %tmp13 = load i64, i64* %tmp12
    store i64 %tmp13, i64* %res.addr.2
    %tmp14 = load i64, i64* %res.addr.2
    ret i64 %tmp14
}
